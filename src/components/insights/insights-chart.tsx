...
export function InsightsChart(props: { settings: GraphSettings; cutoffHour: number }) {
    const { data: taskSets, isLoading: isLoadingTaskSets } = api.taskSet.getForUser.useQuery();
    const { data: dailyLogs, isLoading: isLoadingLogs } = api.dailyLog.getForUser.useQuery();
    ...
    const insightsData =
        taskSets && dailyLogs
            ? calculateInsights(endDayId, props.settings.range, props.settings.resolution, taskSets, dailyLogs)
            : [];
        ...
        return (
            <Card className="overflow-hidden">
                <CardHeader className="flex flex-col gap-1">
                    ...
                    <CardDescription>
                        {props.settings.mode === "relative" ? "Percentage" : "Absolute"} · End{" "}
                        {formatGermanDate(endDayId)}
                    </CardDescription>
                </CardHeader>
                <CardContent className="h-96 px-3 py-3">
                    <div className="text-text-muted flex h-full items-center justify-center text-sm">Loading…</div>
                </CardContent>
            </Card>
        );
    }
